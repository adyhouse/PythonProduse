    ...
    def import_to_woocommerce(self, product_data):
        ...
        try:
            ...
            return False
                ...
                # VERIFICARE POST-CREARE: Confirmă că produsul există cu GET
                try:
                    self.log(f"   🔍 Verificare post-creare (GET /products/{product_id})...", "INFO")
                    verify_response = self.wc_api.get(f"products/{product_id}")
                    ...
                    if verify_response.status_code == 200:
                        ...
                        self.log(f"   ✓ Produs verificat: ID={product_id}, SKU={verified_sku}, Status={verified_status}", "SUCCESS")
                        self.log(f"   ✓ PRODUS CREAT ȘI SALVAT COMPLET!", "SUCCESS")
                        return True
                    else:
                        self.log(f"   ⚠️ AVERTISMENT: Produs creat (201) dar verificare eșuată ({verify_response.status_code})", "WARNING")
                        self.log(f"   ⚠️ Posibil produs parțial creat în baza de date!", "WARNING")
                        return True  # Considerăm succes totuși
                        
                except Exception as verify_error:
                    self.log(f"   ⚠️ Nu am putut verifica produsul după creare: {verify_error}", "WARNING")
                    return True  # Considerăm succes dacă am primit 201
            else:
                # Afișează detalii complete despre eroare
                self.log(f"   ✗ Eroare WooCommerce: Status {response.status_code}", "ERROR")
                
                try:
                    error_data = response.json()
                    if 'message' in error_data:
                        self.log(f"   📝 Mesaj: {error_data['message']}", "ERROR")
                    if 'data' in error_data and 'status' in error_data['data']:
                        self.log(f"   📝 Detalii: {error_data['data']}", "ERROR")
                except:
                    error_text = response.text[:500]
                    self.log(f"   📝 Răspuns server: {error_text}", "ERROR")
                
                # Sugestii rezolvare erori comune
                if response.status_code == 400:
                    self.log(f"   💡 Date invalide - verifică:", "WARNING")
                    self.log(f"      - SKU duplicat (șterge produsul existent)", "WARNING")
                    self.log(f"      - Preț invalid", "WARNING")
                    self.log(f"      - Câmpuri obligatorii lipsă", "WARNING")
                elif response.status_code == 401:
                    self.log(f"   💡 Verifică API Keys (Consumer Key/Secret)", "WARNING")
                elif response.status_code == 403:
                    self.log(f"   💡 Verifică permisiunile API (Read/Write)", "WARNING")
                elif response.status_code == 500:
                    self.log(f"   💡 Eroare server - verifică:", "WARNING")
                    self.log(f"      - Plugin-uri conflictuale", "WARNING")
                    self.log(f"      - Memory limit PHP", "WARNING")
                    self.log(f"      - Log-uri WordPress (wp-content/debug.log)", "WARNING")
